...
for seqId in codons:
    ...
    for frame in sorted(codons[seqId]):
        ...
        aaList = [ translation_table[codon] for codon in codons[seqId][frame] ]
        aaString = ''.join(aaList)
        print(f'{seqId}-frame-{frame+1}-codons\n{codonsString}\nAA:{aaString}')
        for peptide in re.findall(r"M.+?\*",aaString):
#            print(f'{len(peptide)},{longestLength},{longestPeptide},{peptide}')
            if len(peptide) > longestLength:
                longestPeptide=peptide
                longestLength=len(peptide)
    print(f'{longestPeptide}')
